class DateTime {
	...
	_setTime() {
		...
		if (this._twentyFourMode === true) {
			...
			this._greeterClock.innerText = `${hour}:${min}:${sec}`;
		} else {
			// 12-hour mode
			...
			this._greeterClock.innerText = `${hour}:${min}:${sec} ${midDay}`;
		}
		this._sidebarDate.innerText = `${this._daysArr[date.getDay()]}, ${this._monthsArr[date.getMonth()]} ` + `${this._appendZero(date.getDate())}, ${date.getFullYear()}`;

		this._greeterDate.innerText = `${this._monthsArr[date.getMonth()]} ` + `${this._getDayOrdinal(this._appendZero(date.getDate()))}, ${date.getFullYear()}`
			;
		this._greeterMessage.innerText = `Hii there,
			Good ${greeterSuffix}!`;
	}
